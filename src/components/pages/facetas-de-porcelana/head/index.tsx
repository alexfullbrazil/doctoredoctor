import Head from "next/head";

export default function CustomHead() {
  return (
    <Head>
      <title>Facetas Dentais de Porcelana | Doctor&doctor</title>
      <meta
        name="description"
        content="Com as Facetas Dentais de Porcelana é possível recuperar a beleza e a genuinidade do seu sorriso."
      />
      <meta property="og:site_name" content="Doctor&doctor" />
      <meta
        property="og:title"
        content="Facetas Dentais de Porcelana | Doctor&doctor"
      />
      <meta
        property="og:description"
        content="Com as Facetas Dentais de Porcelana é possível recuperar a beleza e a genuinidade do seu sorriso."
      />
      <meta
        property="og:url"
        content="https://esteticadental.doctoredoctor.com.br/lentes-de-porcelana"
      />
      <meta property="og:type" content="website" />
      <meta
        property="og:image"
        content="https://esteticadental.doctoredoctor.com.br/assets/images/og/og-image-lentes-de-porcelana.jpg"
      />
      <meta
        property="og:image:secure_url"
        content="https://esteticadental.doctoredoctor.com.br/assets/images/og/og-image-lentes-de-porcelana.jpg"
      />
      <meta property="og:image:width" content="1200" />
      <meta property="og:image:height" content="600" />
    </Head>
  );
}
